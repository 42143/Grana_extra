...
if(isset($_GET["id"])){
	...
	$id_orcamento = $_GET["id"];
		$d_prop = mysqli_query($con,"SELECT a.id_orcamento,a.valor,c.nome_projeto FROM orcamento AS a INNER JOIN cadastro AS b  
		INNER JOIN cria_projeto AS c ON a.id_usuario = b.id_usuario AND a.id_projeto = c.id_projeto WHERE a.id_orcamento_ind ='$id_orcamento'");
		$prop = mysqli_fetch_array($d_prop);
			
			$verif_boleto = mysqli_query($con,"SELECT id_boleto FROM boleto WHERE id_usuario = '$id' AND id_orcamento = '$prop[id_orcamento]'");
				...
				$id_boleto = mysqli_fetch_row($verif_boleto);
					...
					if($ver == 0){
						...
						mysqli_query($con,"INSERT INTO boleto VALUES(NULL,'$data','$vencimento','$id','0','$prop[id_orcamento]')"); 
					}else{
						date_default_timezone_set("America/sao_Paulo");
						$data = date("Y-m-d");
							$dias_de_prazo_para_pagamento = 5;
							$vencimento = date("Y-m-d",time() + ($dias_de_prazo_para_pagamento * 86400));
						mysqli_query($con,"UPDATE boleto SET data_boleto = '$data',vencimento = '$vencimento' WHERE id_boleto = '$id_boleto[0]' "); 
					}
	// +----------------------------------------------------------------------+
	...
	*/
	// SEUS DADOS
	$dadosboleto["identificacao"] = "$prop[nome_projeto]";
